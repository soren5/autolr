...
class TINY_IMAGENET_CUSTOM_Evaluator(Evaluator):
    def __init__(self, params, configuration_file='TINY_IMAGENET_CUSTOM_CONFIG', task_name='tiny_imagenet_custom', benchmark_data=False):
    ...
    def _init_dataset_(self, validation_size, fitness_size, run, normalize, subtract_mean, benchmark_data=False, test_size=None):
        self.dataset = TINY_IMAGENET_CUSTOM_Dataset(validation_size=validation_size, fitness_size=fitness_size, seed=run, normalize=normalize, subtract_mean=subtract_mean)
        if benchmark_data:
