...
def train_model(phen):
    ...
    for i in range(5):
        ...
        if test_score[-1].item() < final_score:
            final_score = test_score[-1].item()
            final_info = pain
    print("final fitness: ", final_score)
    return final_score, pain
train_model('alpha_func, beta_func, sigma_func, grad_func = lambda shape,  alpha, grad: tf.math.subtract(tf.math.subtract(tf.constant(9.97895596e-01, shape=shape, dtype=tf.float32), tf.math.add(tf.math.add(tf.math.pow(alpha, tf.math.pow(grad, grad)), tf.constant(2.10440443e-03, shape=shape, dtype=tf.float32)), tf.math.add(tf.constant(1.86692945e-04, shape=shape, dtype=tf.float32), tf.math.pow(tf.constant(8.47764177e-01, shape=shape, dtype=tf.float32), grad)))), alpha), lambda shape,  alpha, beta, grad: tf.math.subtract(tf.math.add(tf.math.sqrt(tf.math.sqrt(grad)), beta), tf.math.multiply(beta, alpha)), lambda shape,  alpha, beta, sigma, grad: alpha, lambda shape,  alpha, beta, sigma, grad: tf.math.multiply(grad, tf.constant(5.25231076e-01, shape=shape, dtype=tf.float32))')
#train_model('alpha_func, beta_func, sigma_func, grad_func = lambda shape,  alpha, grad: tf.math.subtract(tf.math.subtract(tf.constant(9.97895596e-01, shape=shape, dtype=tf.float32), tf.math.add(tf.math.add(tf.math.pow(alpha, tf.math.pow(grad, grad)), tf.constant(2.10440443e-03, shape=shape, dtype=tf.float32)), tf.math.add(tf.constant(1.86692945e-04, shape=shape, dtype=tf.float32), tf.math.pow(tf.constant(8.47764177e-01, shape=shape, dtype=tf.float32), grad)))), alpha), lambda shape,  alpha, beta, grad: tf.math.subtract(tf.math.add(tf.math.sqrt(tf.math.sqrt(grad)), beta), tf.math.multiply(beta, alpha)), lambda shape,  alpha, beta, sigma, grad: alpha, lambda shape,  alpha, beta, sigma, grad: tf.math.multiply(grad, tf.constant(5.25231076e-01, shape=shape, dtype=tf.float32))')
