...
import json
import os
from bee_bot.flower import create_report, get_progress_data
...
def bee_report(generation, pop, start_time):
    fitness_samples = [i['fitness'] for i in pop]
    progress_data = get_progress_data(generation - params["RESUME"], params["GENERATIONS"] - params["RESUME"], start_time)
    create_report('bee_report.json', str(params['EXPERIMENT_NAME']) + '/run_' + str(params['RUN']) + '/', 
        {'Generation:': generation,
        'Fitness:': np.min(fitness_samples),
        'Mean:': np.mean(fitness_samples),
        "Standard Deviation:": np.std(fitness_samples),
        "Time Left:": progress_data['time_left'],
        "Time Elapsed:": progress_data['time_elapsed'],
