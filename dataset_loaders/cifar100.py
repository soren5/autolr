...
class CIFAR100_Dataset:
    def __init__(self, validation_size=3500, fitness_size=3500, seed=0, normalize=True, subtract_mean=True, path=None):
        ...
        if path is None:
            ...
            self.path = os.path.join(params['DATA_DIR'], 'cifar100')
        else:
            self.path = path
    ...
    def load_data_for_evolution(self):
        ...
        # CIFAR100 test set is not used in the evolution, it is only used in the final benchmark evaluation, so we can ignore it here.
        
        # Before downloading the dataset, let's check if $DATA_DIR/cifar100 already exists to avoid unnecessary downloads
        if not os.path.exists(self.path):
            print(f"Dataset not found at {self.path}. Downloading CIFAR100 dataset...")
            (x, y), (x_, y_) = cifar100.load_data()
            ...
            print(f"Dataset found at {self.path}. Loading CIFAR100 dataset from local storage...")
    ...
    def load_data_for_benchmark(self):
        # Load the CIFAR100 dataset, split it into training, validation sets.
        # Load the test set of CIFAR100, separately.
