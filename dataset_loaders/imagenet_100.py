...
class IMAGENET_100_Dataset:
    ...
    def load_data_for_evolution(self):
        # IMAGENET_100 dataset is not available in Keras, so we use our own implementation to load it.

        # Load the IMAGENET_100 dataset, split it into training, validation and fitness sets, and preprocess when applicable.
        # IMAGENET_100 test set is not used in the evolution, it is only used in the final benchmark evaluation, so we can ignore it here.
    ...
    def load_data_for_benchmark(self):
        # Load the IMAGENET_100 dataset, split it into training, validation sets.
        # Load the test set of IMAGENET_100, separately.
    ...
    def load_data(self):
        # This expects the IMAGENET_100 dataset to be in a data directory, in a folder called imagenet_100, with the following structure:
        # By default, self.path = 'data/imagenet_100'
        # imagenet_100/train/class_x/xxx.JPEG
        # imagenet_100/val/class_x/xxx.JPEG
        ...
        # We use "IMAGENET_100 Training" for evolution data: training, validation and fitness sets.
        # We use "IMAGENET_100 Validation" for benchmark data: test set.
