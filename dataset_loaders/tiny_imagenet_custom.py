...
class TINY_IMAGENET_CUSTOM_Dataset:
    def __init__(self, validation_size=3500, fitness_size=3500, seed=0, normalize=True, subtract_mean=True, path=None):
        ...
        if path is None:
            ...
            self.path = os.path.join(params['DATA_DIR'], 'tiny_imagenet_custom')
        else:
            self.path = path
    ...
    def load_data(self):
        # This expects the custom TINY_IMAGENET dataset to be in a data directory,
        # in a folder called tiny_imagenet_custom, with the following structure:
        # By default, self.path = 'data/tiny_imagenet_custom'
        # tiny_imagenet_custom/train/class_x/xxx.JPEG
        # tiny_imagenet_custom/val/class_x/xxx.JPEG
