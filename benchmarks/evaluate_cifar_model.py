...
def evaluate_cifar_model(dataset=None, model=None, optimizer=None, batch_size=1000, epochs=100, step=100, verbose=0, experiment_name='development_results', save_best_only=False):
    ...
    if model == None:
        n_model = load_model(os.path.join(models_dir, 'cifar_model.h5'), compile=False)
        print(n_model.summary())
        model = n_model

    model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
        filepath=os.path.join(models_dir, 'checkpoints', f'cifar_model_checkpoint_{experiment_name}.h5'),
    ...
    while epoch_progress < epochs:
        ...
        if save_best_only:
            model.load_weights(os.path.join(models_dir, 'checkpoints', f'cifar_model_checkpoint_{experiment_name}.h5'))
...
def resume_cifar_model(dataset=None, optimizer=None, batch_size=1000, epochs=100, verbose=0):  
    ...
    n_model = load_model(os.path.join(models_dir, 'cifar_model.h5'), compile=False)
    model = n_model 
    model.load_weights(os.path.join(models_dir, 'checkpoints', 'cifar_model_checkpoint.h5'))
